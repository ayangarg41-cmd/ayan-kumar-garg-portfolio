...
if (researchTerminal) {
  const terminalData = {
    tata: {
      name: 'Tata Technologies', code: 'TATATECH', sector: 'Engineering & Digital Services', price: 'Rs 719', priceLabel: 'CMP as on July 10, 2026',
    ...
    havells: {
      name: 'Havells India', code: 'HAVELLS', sector: 'Electrical Consumer Durables & FMEG', price: 'Rs 1,190.6', priceLabel: 'CMP as on Mar 2026',
  ...
  function renderOnePagerExplorer() {
    ...
    const logo = explorerCompany === 'tata' ? 'assets/logos/tata-technologies.svg' : 'assets/logos/havells.svg';
    const views = {
      snapshot: `<div class="explorer-snapshot"><article class="explorer-story"><span class="terminal-label">Company at a glance</span><p>${data.onePager.description}</p><dl><div><dt>Founded</dt><dd>${data.onePager.founded}</dd></div><div><dt>Headquarters</dt><dd>${data.onePager.headquarters}</dd></div><div><dt>Sector</dt><dd>${data.sector}</dd></div></dl><div class="explorer-focus"><span class="terminal-label">Research priorities</span>${data.onePager.watch.map(([item,reason],index) => `<p><b>0${index+1}</b><span><strong>${item}</strong><small>${reason}</small></span></p>`).join('')}</div></article><article class="explorer-scoreboard"><span class="terminal-label">FY26 financial snapshot</span><div><p><small>Revenue</small><strong>${crore(data.revenue)}</strong></p><p><small>Net profit</small><strong>${crore(data.profit)}</strong></p><p><small>ROCE</small><strong>${data.roce.toFixed(1)}%</strong></p><p><small>P/E</small><strong>${data.pe.toFixed(1)}x</strong></p></div><aside><strong>What the numbers indicate</strong><p>${data.onePager.performance}</p></aside></article></div>`,
      business: `<div class="explorer-business"><article><span class="terminal-label">Revenue mix</span><div class="explorer-mix-bar">${data.mix.map(([label,value],index) => `<i style="width:${value}%" data-label="${label} ${value}%" class="mix-segment-${index}"></i>`).join('')}</div><div class="explorer-mix-legend">${data.mix.map(([label,value],index) => `<p><i class="mix-key-${index}"></i><span>${label}</span><strong>${value}%</strong></p>`).join('')}</div><aside><strong>Why the mix matters</strong><p>The mix shows where revenue concentration sits and which operating drivers have the greatest influence on growth, margin and risk.</p></aside></article><article><span class="terminal-label">Competitive strengths—with evidence</span><ol>${data.onePager.strengths.map(([item,reason],index) => `<li><b>0${index+1}</b><span><strong>${item}</strong><small>${reason}</small></span></li>`).join('')}</ol></article></div>`,
    ...
    container.innerHTML = `<div class="explorer-company-banner"><div class="explorer-logo"><img src="${logo}" alt="${data.name} logo" /></div><div><span>${data.code}</span><h4>${data.name}</h4><p>${data.priceLabel}: ${data.price}</p></div><a href="${data.profile}" target="_blank" rel="noopener">Open full one-pager ↗</a></div><div class="explorer-tabs" role="tablist" aria-label="One-pager views"><button type="button" data-explorer-view="snapshot" class="${explorerView === 'snapshot' ? 'is-active' : ''}" aria-selected="${explorerView === 'snapshot'}">Snapshot</button><button type="button" data-explorer-view="business" class="${explorerView === 'business' ? 'is-active' : ''}" aria-selected="${explorerView === 'business'}">Business model</button><button type="button" data-explorer-view="lens" class="${explorerView === 'lens' ? 'is-active' : ''}" aria-selected="${explorerView === 'lens'}">Investment lens</button></div><div class="explorer-view">${views[explorerView]}</div>`;
  }
